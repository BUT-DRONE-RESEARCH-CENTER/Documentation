...
# Apply the Leopard Imaging IMX900 driver patch
patch -p1 < imx900_R36.4.3_Orin_NX_driver.patch
...
with open("LI-IMX900-MIPI-FR-EO_Driver_Guide.md", "w") as f:
...
[file-tag: code-generated-file-0-1783944211962982543]

Here is the fully formatted Markdown text parsed directly from the document:
...
[cite_start]This driver guide describes the setup and installation process for the **LI-IMX900-MIPI-FR-EO V1.0** camera kit on the **NVIDIA Jetson Orin NX Developer Kit**[cite: 9].

### Key Features
* [cite_start]**Supported Camera:** 1 × LI-IMX900-MIPI-FR-EO V1.0 camera [cite: 9]
* [cite_start]**Resolution & Frame Rate:** 2064×1552 @ 30fps [cite: 9]
* [cite_start]**Driver Base Linux Version:** L4T R36.4.3 (JetPack 6.2) [cite: 9]
* [cite_start]**Default Port:** The camera connects to the **CAM1** interface (maps to `/dev/video0`)[cite: 11, 14].
...
| Category | Component Description | Quantity |
| :--- | :--- | :--- |
| **Platform** | NVIDIA Jetson Orin NX Developer Kit | [cite_start]1 [cite: 9] |
| **Camera** | LI-IMX900-MIPI-FR-EO V1.0 | [cite_start]1 [cite: 9] |
| **Cable** | FAW-1233 | [cite_start]2 [cite: 9] |
| **Adapter / Carrier Board** | LI-FPC22-IPEX-PI V1.0 | [cite_start]1 [cite: 9] |
| **Adapter / Carrier Board** | LI-MIPI-IPEX-20-30-ADP V1.0 | [cite_start]1 [cite: 9] |
| **Power Supply** | 19V DC Power Supply | [cite_start]1 [cite: 9] |
...
| Revision | Description | Release Date | Author | Tested By |
| :--- | :--- | :--- | :--- | :--- |
| **20251015** | SVN version updates / Documentation review | 10/15/2025 | Jueming Lin | [cite_start]— [cite: 12] |
| **20241015** | First Release | 10/15/2025 | Jueming Lin | [cite_start]— [cite: 12] |

### Known Bugs / Issues
1. [cite_start]**LI-FPC22-IPEX-PI** requires a hardware rework[cite: 12].
...
### Step 1: Driver Installation (Host PC Setup)
[cite_start]Perform these steps on an **Intel x64 Host PC** running **Ubuntu 20.04 64-bit OS**[cite: 13].

1. [cite_start]Download the R36.4.3 Driver Package (BSP), Sample Root Filesystem, and Driver Package Sources from the NVIDIA Developer website[cite: 13]:
   * [cite_start][NVIDIA Jetson Linux 36.4.3 Page](https://developer.nvidia.com/embedded/jetson-linux-r3643) [cite: 13]

2. [cite_start]Extract the files and assemble the root filesystem (`rootfs`) using the following commands[cite: 13]:
   ```bash
   # Extract the Driver Package
   sudo tar xpf Jetson_Linux_R36.4.3_aarch64.tbz2
...
Note: Upon loading, CAM1 maps directly to video0 (`/dev/video0`).
...
2. Install development dependencies on the Jetson Orin NX:
   ```bash
   sudo apt-get update
   sudo apt-get install cmake build-essential pkg-config libx11-dev libgtk-3-dev libexpat1-dev libjpeg-dev libgstreamer1.0-dev
   ```
...
4. Build and install the Argus camera sample application:
   ...
   sudo apt install -y pkg-config libx11-dev libgtk-3-dev libexpat1-dev libjpeg-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev
   ```

   ```bash
   cd ~/leopard_jetson_configured/usr/src/jetson_multimedia_api/argus/cmake
   cmake ..
   make
   sudo make install
   ```
...
# Capture a test stream stream sequence
v4l2-ctl -V -v width=2064,height=1552,pixelformat=BA10 --set-ctrl bypass_mode=0 --set-ctrl sensor_mode=0 --stream-mmap --stream-count=100 -d /dev/video0
```

### Method 3: GStreamer Pipeline

Launch a hardware-accelerated video stream rendering directly to a window display:

```bash
sudo apt-get update
sudo apt-get install nvidia-l4t-gstreamer gstreamer1.0-tools gstreamer1.0-alsa gstreamer1.0-plugins-base gstreamer1.0-plugins-good gstreamer1.0-plugins-bad gstreamer1.0-plugins-ugly gstreamer1.0-libav libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgstreamer-plugins-good1.0-dev libgstreamer-plugins-bad1.0-dev nvidia-l4t-gstreamer   

# Run pipeline
gst-launch-1.0 nvarguscamerasrc sensor-id=0 ! 'video/x-raw(memory:NVMM), width=(int)2064, height=(int)1552, framerate=30/1'  ! nvvidconv flip-method=0 ! 'video/x-raw, format=(string)I420' ! xvimagesink -e
```
...
Follow these instructions if you would like to re-compile the camera kernel driver module from source code.
...
4. Apply the camera patch inside your working directory:
   ...
   4. Customizing Device Tree Configuration (Optional)
   ```bash 
